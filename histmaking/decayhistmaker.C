...
void decayhistmaker(int section = 0, const char * type = "MB")
{
  ...
  const char * wfilename = (section == -1) ? Form("hists/decay_mass_%s.root",type) : Form("hists/decay_mass_%s_%i.root",type,section);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if (truthpar_n == 0) continue;
    if (fabs(vz) > 30) continue;
    float maxpt = 0;
    // For truth pT distributions
    ...
    hmaxpt->Fill(maxpt);  
    // For knowing if a decay happened
    bool founddecay = false;
    for (int it = 0; it < truthpar_n; it++) {
      auto pho = *(TLorentzVector*) truth_diphoton_4mom->At(it);
      if (truth_id[it] == 111 && truth_found_decay1 && truth_found_decay2) founddecay=true;
      if (truth_id[it] == 221 && truth_found_decay1 && truth_found_decay2) founddecay=true;
    }
    if (!founddecay) continue;

...
void alphafitmass(const char * particle = "pi0", const char * sample = "data", const char * trigger = "MB") {
  set_maps();
  ...
  for (int pt = 0; pt < 3; pt++) { 
    for (int alph = 0; alph < 3; alph++) {
      ...
      TFile * wf = new TFile(Form("datafits_smeartest/workspace_pt%i_a%i.root",pt,alph),"RECREATE"); 
      RooWorkspace * ws;

...
void drawetadata()
{
  ...
  int pointcount=0;
  for(int i = 1; i < 7; ++i) {
    TFile *f = new TFile(Form("/sphenix/user/samfred/run25/ppg11/histmaking/sufficient/workspace_fits_data_%s_pt%i.root",particle.c_str(),i),"read");
  ...
  gmc_mean->GetYaxis()->SetLimits(0.3,0.7);
  gmc_mean->GetYaxis()->SetRangeUser(0.5,0.75);
  gmc_res->GetYaxis()->SetLimits(0.0,0.2);
  gmc_res->GetYaxis()->SetRangeUser(0.0,0.2);
  gmc_ratio->GetYaxis()->SetLimits(0.0,0.5);
  gmc_ratio->GetYaxis()->SetRangeUser(0.02,0.22);
  ...
  gmc_mean->Draw("AP");
  gmc_mean->GetXaxis()->SetTitle("p_{T}^{#eta} [GeV]");
  ...
  drawText("#eta",sPHENIX_posx,sPHENIX_posy-posy_diff*6,1,18);
  ...
  c1->SaveAs("plot_eta_peak_data.pdf");
  ...
  gmc_res->Draw("AP");
  gmc_res->GetXaxis()->SetTitle("p_{T}^{#eta} [GeV]");
  ...
  gmc_ratio->Draw("AP");
  gmc_ratio->GetXaxis()->SetTitle("p_{T}^{#eta} [GeV]");

...
void Fun4All_macro_mc(const char * inqueue="queue_run28_v00001.list", const char * trigger = "MB", bool istest = true) {
  ////////////////////
  // File management
  ////////////////////

  string infile = Form("/sphenix/user/samfred/run25/ppg11/treemaking/macros/filelists/pythia_%s/%s",trigger,inqueue);
  ...
  string outdir = Form("/sphenix/tg/tg01/jets/samfred/run25/pythia_%s",trigger);
  ...
  if (istest) outfile = Form("test_%s.root",inqueue);
  ...
  ClusterProfile->set_min_cluster_e(1);
  ClusterProfile->set_inputNodeName("CLUSTERINFO_CEMC");
  ...
  ClusterProfileMother->set_min_cluster_e(1);
